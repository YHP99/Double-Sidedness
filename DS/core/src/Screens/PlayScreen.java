...
public class PlayScreen implements Screen{
	...
	private WorldContactListener world_contact;
	private World world;
	...
	public PlayScreen(MyGdxGame game, boolean is_first) {
		//사운드 관련 소스코드
		MusicPlayer.BackMusicStop();
		MusicPlayer.MainBackgMusic("Music/Stage1.mp3");
		...
		world_contact = new WorldContactListener();
	...
	public void update(float dt) {
		...
		{
			if (world_contact.dialog == 2) { m = ment.ment2(); }
			if (world_contact.dialog == 3) { m = ment.ment3(); }
			if (world_contact.dialog == 4) { m = ment.ment4(); }
			if (world_contact.dialog == 5) { m = ment.ment5(); }
