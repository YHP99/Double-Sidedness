...
import Sprite.Goomba4;

public class B2WorldCreator4 {
	private MyGdxGame game;
	private Array<Goomba4> goombas;

	public B2WorldCreator4(PlayScreen4 screen) {
		...
		// 레이어 6번 Enemy object
		goombas = new Array<Goomba4>();
		for (MapObject object : map.getLayers().get(6).getObjects().getByType(RectangleMapObject.class)) {
			Rectangle rect = ((RectangleMapObject) object).getRectangle();
			goombas.add(new Goomba4(screen, (rect.getX() + rect.getWidth() / 2) / game.PPM,
					(rect.getY() + rect.getHeight() / 2) / game.PPM));
		}
	...
	public Array<Goomba4> getGoombas() {
		return goombas;
	}
